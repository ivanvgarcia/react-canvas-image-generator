...
import { getAvatars, chooseAvatar, addChosenAvatar, removeChosenAvatar } from 'actions/avatar';
...
const AvatarCanvas = props => {
  ...
  const chooseAvatars = clickedAvatar => {
    ...
    if (hasAvatar)
      return dispatch(removeChosenAvatar(clickedAvatar._id))
  ...
  if (isMobile) return <h1>Sorry, you need to be on a mobile device to access this feature!</h1>
